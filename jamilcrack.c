 ...
 *  Copyright (C) 2008, 2009, 2010, 2011, 2012, 2013 by muhammad jamil (jamilsoft)@gmail.com
 *  Copyright (C) 2020  Muhammad Jamil by myakububauchi@gmail.com
 * 
 * 
 ...
 *     general works.  I have received word that jamilcrack compiles on MacOS.
...
unsigned long long calc = 0;
  ...
  while (1 != 0) {
    ...
    if (calc < 100)
      fprintf(stderr,"\njamilcrack: %3llu%% completed generating output\n", calc);
  }
...
size_t outlen; /* temp for size of narrow output string */
  ...
  if (ctrlbreak == 1 ) {
    (void)make_narrow_string(gconvbuffer,block2,gconvlen);
    fprintf(stderr,"Jamilcrack ending at %s\n",gconvbuffer);
  }
...
static void usage() {
  fprintf(stderr,"jamilcrack version %s\n\n", version);
  fprintf(stderr,"Jamilcrack can create a wordlist based on criteria you specify.  The outout from jamilcrack can be sent to the screen, file, or to another program.\n\n");
  fprintf(stderr,"Usage: jamilcrack <min> <max> [options]\n");
  fprintf(stderr,"where min and max are numbers\n\n");
  fprintf(stderr,"Please refer to the man page for instructions and examples on how to use jamilcrack.\n");
}
...
//  (void) signal(SIGINFO, printme);
  ...
  if (charset == NULL) {
    fprintf(stderr,"jamilcrack: can't allocate memory for default charset\n");
    exit(EXIT_FAILURE);
  }

  upp_charset = dupwcs(def_upp_charset);
  if (upp_charset == NULL) {
    fprintf(stderr,"jamilcrack: can't allocate memory for default upp_charset\n");
    exit(EXIT_FAILURE);
  }

  num_charset = dupwcs(def_num_charset);
  if (num_charset == NULL) {
    fprintf(stderr,"jamilcrack: can't allocate memory for default num_charset\n");
    exit(EXIT_FAILURE);
  }

  sym_charset = dupwcs(def_sym_charset);
  if (sym_charset == NULL) {
    fprintf(stderr,"jamilcrack: can't allocate memory for default sym_charset\n");
    exit(EXIT_FAILURE);
  }
  ...
  if (argc >= 4) {
    if ((argc > i) && (*argv[i] != '-')) { /*test for ./jamilcrack 1 2 -? */
      if (*argv[i] != '+') {
        ...
        if (tempwcs==NULL || charset==NULL) {
          fprintf(stderr,"jamilcrack: can't allocate memory for user charset\n");
          exit(EXIT_FAILURE);
        }
    ...
    if ((argc > i) && (*argv[i] != '-')) { /*test for ./jamilcrack 1 2 -? */
      if (*argv[i] != '+') {
        ...
        if (tempwcs==NULL || upp_charset==NULL) {
          fprintf(stderr,"jamilcrack: can't allocate memory for upp_charset\n");
          exit(EXIT_FAILURE);
        }
    ...
    if ((argc > i) && (*argv[i] != '-')) { /*test for ./jamilcrack 1 2 -? */
      if (*argv[i] != '+') {
        ...
        if (tempwcs==NULL || num_charset==NULL) {
          fprintf(stderr,"jamilcrack: can't allocate memory for num_charset\n");
          exit(EXIT_FAILURE);
        }
    ...
    if ((argc > i) && (*argv[i] != '-')) { /*test for ./jamilcrack 1 2 -? */
      if (*argv[i] != '+') {
        ...
        if (tempwcs==NULL || sym_charset==NULL) {
          fprintf(stderr,"jamilcrack: can't allocate memory for sym_charset\n");
          exit(EXIT_FAILURE);
        }
  ...
  if (max > MAXSTRING) {
    fprintf(stderr,"Jamilcrack can only make words with a length of less than %d characters\n", MAXSTRING+1);
    exit(EXIT_FAILURE);
  }
  ...
  for (; i<argc; i+=2) { /* add 2 to skip the parameter value */
    ...
    if (strncmp(argv[i], "-e", 2) == 0) {
      if (i+1 < argc) {
        ...
        fprintf(stderr,"Please specify the string you want jamilcrack to stop at\n");
        exit(EXIT_FAILURE);
      }
    }
    ...
    if (strncmp(argv[i], "-o", 2) == 0) {  /* outputfilename specified */
      ...
      if (i+1 < argc) {
        ...
        if (hold == NULL) {
          ...
          if (fpath == NULL) {
            fprintf(stderr,"jamilcrack: can't allocate memory for fpath1\n");
            exit(EXIT_FAILURE);
          }
          ...
          if (fpath == NULL) {
            fprintf(stderr,"jamilcrack: can't allocate memory for fpath2\n");
            exit(EXIT_FAILURE);
          }
...
*/
  ...
  if (saw_unicode_input) {
    char response[8];
    fprintf(stderr,
      "Notice: Detected unicode characters.  If you are piping jamilcrack output\n"\
    ...
    */ /* TODO: must calculate finalfilesize during line calculation */
    ...
    if (flag3 == 0) {
      if (suppress_finalsize == 0) {
        fprintf(stderr,"Jamilcrack will now generate the following amount of data: ");
      ...
      fprintf(stderr,"Jamilcrack will now generate the following number of lines: ");
    ...
    if (flag3 == 0) {
      if (suppress_finalsize == 0) {
        fprintf(stderr,"Jamilcrack will now generate approximately the following amount of data: ");
        fprintf(stderr,"%llu bytes\n",my_thread.finalfilesize);
        fprintf(stderr,"%llu MB\n",my_thread.finalfilesize/1048576);
        fprintf(stderr,"%llu GB\n",my_thread.finalfilesize/1073741824);
        fprintf(stderr,"%llu TB\n",my_thread.finalfilesize/1099511627776);
